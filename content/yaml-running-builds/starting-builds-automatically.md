...
Example 1. This build will continue if the triggering event was *not* a draft pull request update. In other words, it will skip the build if a branch is in a pull request and marked as a draft:
...
Example 2. This build will *not* continue if the pull request branch has a "codemagicTest" label:
...
Example 3. You can also combine triggering events, just make sure that each condition is wraped with brackets:
