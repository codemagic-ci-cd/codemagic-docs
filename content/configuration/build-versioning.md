...
Note that I’ve added `--build-name` & `--build-number` as an example. Don’t pass both arguments to the build command if you’ve committed this to version control in the `pubspec.yaml` version field.
