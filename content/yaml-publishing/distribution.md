...
Post-processing has a two-step timeout. If the uploaded build cannot be found in App Store Connect after 15 minutes, it is likely that Apple has detected an issue with the artifact and post-processing is cancelled. In total, post-processing runs for 60 minutes maximum. If the uploaded build has not exited the processing status by then, post-processing in cancelled. You will be still able to manually submit the build to beta review, upload release notes and distribute the app to beta groups once the build becomes available in App Store Connect.
